...
pub struct AudioInput {
    ...
    agg_desc: Option<arc::Retained<cf::DictionaryOf<cf::String, cf::Type>>>,
    input_device: Option<ca::Device>,
    source: AudioSource,
}
...
impl AudioInput {
    ...
    pub fn stream(self) -> AudioStream {
        match self.source {
            AudioSource::System => {
                let tap = self.tap.expect("Tap should exist for system audio");
                let agg_desc = self
                    .agg_desc
                    .expect("Agg desc should exist for system audio");
                ...
                let device = Self::start_system_device(agg_desc, tap, &mut ctx).unwrap();
...
impl Stream for AudioStream {
    ...
    fn poll_next(
        ...
        let consumer = self.consumer.as_mut().expect("Consumer should exist");
        ...
        if self
            ._ctx
            .as_ref()
            .map(|c| c.should_terminate.load(Ordering::Acquire))
            .unwrap_or(false)
        {
            return match consumer.try_pop() {
...
impl Drop for AudioStream {
    fn drop(&mut self) {
        if let Some(ctx) = self._ctx.as_ref() {
            ctx.should_terminate.store(true, Ordering::Release);
        }
    }
}
