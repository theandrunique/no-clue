use crate::{ai_providers::ProviderSettings, db::get_connection};
use rusqlite::{params, Result};

pub fn upsert_provider(provider: &str, settings: &ProviderSettings) -> Result<()> {
...
pub fn get_provider_settings(provider: &str) -> Result<Option<ProviderSettings>> {
    let conn = get_connection()?;
    ...
    if let Some(row) = rows.next()? {
        ...
        let settings: ProviderSettings = serde_json::from_str(&config_str).map_err(|e| {
